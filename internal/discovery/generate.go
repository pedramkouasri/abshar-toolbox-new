...
func NewGenerator(cnf config.Config, tag1 string, tag2 string, loading contracts.Loader) *discovery {
	...
	return &discovery{
		tempDir:     cnf.TempDir + "/discovery",
		outFile:     cnf.TempDir + "/builds/discovery.tar.gz",
...
func (d *discovery) GetFilePath() string {
	return d.outFile
}

func (d *discovery) Generate(ctx context.Context) error {
...
func (d *discovery) runGenerate(ctx context.Context) error {
	var err error
